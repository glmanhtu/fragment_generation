...
class FragmentizeStrategy:
    ...
    def get_edge(self, width):
        length = 0
        edge = None
        while length < width:
            edge = random.choice(self.edges)
            length = max(edge['width'], edge['height'])
    ...
    def fragmentize(self, patch):
        p_h, p_w = patch.shape[:2]
        top_edge = self.get_edge(p_w)
        top_edge = cv2.rotate(top_edge, cv2.ROTATE_180)
        bottom_edge = self.get_edge(p_w)
        left_edge = self.get_edge(p_h)
        left_edge = cv2.rotate(left_edge, cv2.ROTATE_90_CLOCKWISE)
        right_edge = self.get_edge(p_h)
...
class Fragment1v05Rotate90(FragmentizeStrategy):
